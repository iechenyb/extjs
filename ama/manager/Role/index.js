...
function restPageList($scope,$http){
    ...
    page  = $("#page").page({
        ...
        jump: function(context, first) {
            ...
            $http.get('http://localhost:8088/comweb/common/roles.do?start='+ $scope.start+'&limit=10')
...
var initController = function ( $scope,$http) {
    ...
    $scope.cur = 1;
    $scope.pageSize=10;
    $http.get('http://localhost:8088/comweb/common/roles.do?start='+0+'&limit=10')
        .success(function (data) {
            $scope.items = data.data;
    ...
    $scope.delRole=function (id) {
            ...
            $http.get('http://localhost:8088/comweb/common/delRole.do?ids='+id).success(function(data){
                $http.get('http://localhost:8088/comweb/common/roles.do?start='+ $scope.start+'&limit=10')
                    .success(function (data) {
                        $scope.items = data.data;
                        //当删除数据等于0时，当前页数据没有啦
    ...
    $scope.addRole=function () {
        $http.get('http://localhost:8088/comweb/common/addRole.do?name=1&desc=haha').success(function(data){
            $http.get('http://localhost:8088/comweb/common/roles.do?start='+ $scope.start+'&limit=10')
                .success(function (data) {
                    $scope.items = data.data;
                    $scope.total = data.total;
...
var myApp = angular.module('myApp', ['ui.router',"mytop"])
    ...
    .controller('pageController', function ($scope) {
        $scope.title = "角色管理";
        $scope.bashPath='http://localhost:8088';
    });